...
typedef struct HGPinyinTableCell {
    NSString * pinyin;
    NSString * hanzi;
} HGPinyinTableCell;

const HGPinyinTableCell  pinyinTable[] = {
{@"A",@"啊阿呵吖嗄腌锕錒"},
...
@implementation HGPinyinService
...
+ (NSString *)convertFrom:(NSString *)hanzi {
    ...
    int pinyinCount = sizeof(pinyinTable) / sizeof(HGPinyinTableCell);
    ...
    if (!cache) {
        if (cacheCeil == 0 && cacheFloor == 0xffffffff) {
            ...
            for (int i = 0; i < pinyinCount; ++i) {
                HGPinyinTableCell pycell = pinyinTable[i];
        ...
        cache = malloc(cacheLength * sizeof(int));
        
        for (int i = 0; i < pinyinCount; ++i) {
            HGPinyinTableCell pycell = pinyinTable[i];
            NSString *hanziList = pycell.hanzi;
            int count = [hanziList length];
            for (int j = 0; j < count; j++) {
                unichar c = [hanziList characterAtIndex:j];
    ...
    int hzLength = [hanzi length];
    for (int i = 0; i < hzLength; i++) {
        unichar c = [hanzi characterAtIndex:i];
        NSInteger cell = c - cacheFloor;
        if (cell < 0 || cell > cacheCeil - cacheFloor) {
            [string appendFormat:@"%c", c];
        } else {
            int pos = cache[cell];
            if (0 <= pos && pos < pinyinCount) {
                HGPinyinTableCell pycell = pinyinTable[pos];

...
- (id)initPayWithRenren:(RenrenService*)renren andSecretKey:(NSString *)secret andLocalMem:(BOOL)isUsed
{
    self = [super init];
    if (self) {
...
- (void)checkAppPayStatus
{
    ...
    [calcStr appendFormat:@"%@%@%@",RENREN_APP_ID,dateString,self.appSecret];
    
    NSString *checkCode = [ROUtility md5HexDigest:calcStr];
    
    NSMutableDictionary *params = [NSMutableDictionary dictionaryWithObjectsAndKeys:RENREN_APP_ID,@"app_id",
                                   dateString,@"time",
...
- (NSString*)getOrderNumber
{
    ...
    NSString *appIDHeadFourChar = [[NSString stringWithFormat:RENREN_APP_ID] substringWithRange:NSMakeRange(0,4)];
...
- (NSMutableDictionary*)prepareRepairUrlParams:(ROPayOrderInfo *)order
{
    NSMutableDictionary *params = [NSMutableDictionary dictionaryWithObjectsAndKeys:RENREN_APP_ID,@"app_id",self.renren.accessToken,@"access_token",nil];
...
{
    NSMutableString *calcStr = [[[NSMutableString alloc] init] autorelease];
    [calcStr appendFormat:@"%@%@%@%@",RENREN_APP_ID,order.orderNum,order.orderTime,self.appSecret];
    NSString *appCode = [ROUtility md5HexDigest:calcStr];
    
    NSMutableDictionary *params = [NSMutableDictionary dictionaryWithObjectsAndKeys:
                                   RENREN_APP_ID,@"app_id",
                                   appCode,@"app_encode",
               ...
               andParentController:(UIViewController *)controller
{
    ...
    if (![self.renren isSessionValid]) {
        ...
        [parameters setValue:RENREN_APP_KEY forKey:@"client_id"];

...
@interface RenrenService(Private)
...
static RenrenService *sharedRenren = nil;
...
+ (RenrenService *)newRenRen{
    RenrenService *newRenrenObject = [[RenrenService alloc] init];
    [newRenrenObject isSessionValid];
    newRenrenObject.appKey = RENREN_APP_KEY;
    newRenrenObject.appId = RENREN_APP_ID;
    return [newRenrenObject autorelease];
}

+ (RenrenService *)sharedRenren {
    if (!sharedRenren) {
        sharedRenren = [[RenrenService alloc] init];
        [sharedRenren isSessionValid];
        sharedRenren.appKey = RENREN_APP_KEY;
        sharedRenren.appId = RENREN_APP_ID;
    }
    return sharedRenren;
}
...
- (void)checkOperation:(ROResponse*)operateResponse operateType:(RODialogOperateType )operateType{
    NSDictionary* authDictionary = nil;
    ROError* authError = nil;
    switch (operateType) {
        case RODialogOperateSuccess:
            
            authDictionary = operateResponse.rootObject;
        ...
        case RODialogOperateFailure:
            authError = operateResponse.error;
...
-(void)addCommentToStatus:(ROStatusAddCommentRequestParam *)param andDelegate:(id<RenrenDelegate>)delegate {
    if (![param.method isEqualToString:@"status.addComment"]) {
		NSLog(@"API Method Error!");
		return;
    }
...
-(void)addCommentToPhoto:(ROPhotoAddCommentRequestParam *)param andDelegate:(id<RenrenDelegate>)delegate {
    if (![param.method isEqualToString:@"photos.addComment"]) {
		NSLog(@"API Method Error!");
		return;
    }
...
-(void)addCommentToBlog:(ROBlogAddCommentRequestParam *)param andDelegate:(id<RenrenDelegate>)delegate {
    if (![param.method isEqualToString:@"blog.addComment"]) {
		NSLog(@"API Method Error!");
		return;
    }
...
-(void)forwardStatus:(ROForwardStatusRequestParam *)param andDelegate:(id<RenrenDelegate>)delegate {
    if (![param.method isEqualToString:@"status.forward"]) {
		NSLog(@"API Method Error!");
		return;
    }
...
-(void)share:(ROShareRequestParam *)param andDelegate:(id<RenrenDelegate>)delegate {
    if (![param.method isEqualToString:@"share.share"]) {
		NSLog(@"API Method Error!");
		return;
    }

...
- (WeiBoComment*)initWithJsonDictionary:(NSDictionary*)dic {
	...
	if ((self = [super init])) {
		...
		if (statusDic) {
			status = [[WeiBoStatus statusWithJsonDictionary:statusDic] retain];
		}
		
		NSDictionary* replyCommentDic = [dic objectForKey:@"reply_comment"];
		if (replyCommentDic) {
			replyComment = [[WeiBoComment commentWithJsonDictionary:replyCommentDic] retain];
		}
...
+ (WeiBoComment*)commentWithJsonDictionary:(NSDictionary*)dic {
	return [[[WeiBoComment alloc] initWithJsonDictionary:dic] autorelease];
}

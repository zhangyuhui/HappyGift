...
static WBEngine *sharedWeibo = nil;
...
+ (WBEngine*)sharedWeibo {
    if (sharedWeibo == nil) {
        sharedWeibo = [[WBEngine alloc] initWithAppKey:kWBAppKey appSecret:kWBAppSecret];
        [sharedWeibo setRedirectURI:kWBCallbackURL];
        [sharedWeibo setIsUserExclusive:NO];
        WBAuthorize *auth = [[WBAuthorize alloc] initWithAppKey:kWBAppKey appSecret:kWBAppSecret];
        [auth setDelegate:sharedWeibo];
        [auth setRedirectURI:kWBCallbackURL];
        sharedWeibo.authorize = auth;
        [auth release];
    }
    return sharedWeibo;
}
...
- (void)sendWeiBoWithText:(NSString *)text imageData:(NSData *)image {
...
- (void)followUser:(NSString*)userId andUserName:(NSString*)userName {
...
- (void)commentTweet:(NSString*)tweetId withComment:(NSString*)comment {
    NSMutableDictionary *params = [NSMutableDictionary dictionary];
...
- (void)repostTweet:(NSString*)tweetId withStatus:(NSString*)status {
    NSMutableDictionary *params = [NSMutableDictionary dictionary];
    [params setValue:tweetId forKey:@"id"];
